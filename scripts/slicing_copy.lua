...
local function cut(shift, endpos)
    ...
    cmds:arg("-c:s", "copy")
    cmds:arg("-map", string.format("v:%s?", mp.get_property_number("current-tracks/video/id", 1) - 1))
    cmds:arg("-map", string.format("a:%s?", mp.get_property_number("current-tracks/audio/id", 1) - 1))
    cmds:arg("-map", string.format("s:%s?", mp.get_property_number("current-tracks/sub/id", 1) - 1))
...
local function startcutting()
    local startpos, endpos = mp.get_property_number("ab-loop-a"), mp.get_property_number("ab-loop-b")
    if startpos == nil then
        info("Please set ab-loop")
        return
    end
    info(string.format("Cut fragment: %s-%s", timestamp(startpos), timestamp(endpos)))
    cut(startpos, endpos)
end
...
mp.add_key_binding("c", "startcutting", startcutting)
